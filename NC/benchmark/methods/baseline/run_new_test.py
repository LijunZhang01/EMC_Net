...
from model.GNN_test import myGAT
import dgl
...
def run_model_DBLP(args):
    ...
    for _ in range(args.repeat):
        ...
        # training loop
        net.train()
        early_stopping = EarlyStopping(patience=args.patience, verbose=True, save_path='./NC/benchmark/methods/baseline/checkpoint/checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers))
        
        # testing with evaluate_results_nc
        net.load_state_dict(torch.load('./NC/benchmark/methods/baseline/checkpoint/checkpoint_{}_{}_zhanshi.pt'.format(args.dataset, args.num_layers)))
        net.eval()
        test_logits = []
        with torch.no_grad():
            logits,_,embedding = net(features_list,e_feat,feat_drop_idx,src_node_type,mask_list,feat_keep_idx,device,emb,type_mask,args)
            ...
            X = embedding[mask_list[0]][test_idx].cpu().numpy()
            print(X.shape)
            np.save("embedding_{}.npy".format(args.dataset),X)
            np.save("color_{}.npy".format(args.dataset),color)
            print(color.shape)
            # t0 = time()
