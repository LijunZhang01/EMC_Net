...
def run_model_DBLP(args,feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
                   num_epochs, patience, batch_size, neighbor_samples, repeat, save_postfix):
    adjlists, edge_metapath_indices_list, features_list, adjM, type_mask, labels, train_val_test_idx, dl ,emb= load_DBLP_data()
    ...
    for _ in range(repeat):
        net = MAGNN_nc_mb(3, 6, etypes_list, in_dims, hidden_dim, out_dim, num_heads, attn_vec_dim, emb.size()[1],rnn_type, dropout_rate)
        net.to(device)
        ...
        for epoch in range(num_epochs):
            ...
            for iteration in range(train_idx_generator.num_iterations()):
                ...
                logits, embeddings = net(
                    (train_g_list, features_list, type_mask, train_indices_list, train_idx_batch_mapped_list),args,emb,mask_list,src_node_type,feat_keep_idx,feat_drop_idx)
            ...
            with torch.no_grad():
                for iteration in range(val_idx_generator.num_iterations()):
                    ...
                    logits, embeddings = net(
                        (val_g_list, features_list, type_mask, val_indices_list, val_idx_batch_mapped_list),args,emb,mask_list,src_node_type,feat_keep_idx,feat_drop_idx)
                    logp = F.log_softmax(logits, 1)
        ...
        with torch.no_grad():
            for iteration in range(test_idx_generator.num_iterations()):
                ...
                logits, embeddings = net((test_g_list, features_list, type_mask, test_indices_list, test_idx_batch_mapped_list),args,emb,mask_list,src_node_type,feat_keep_idx,feat_drop_idx)
            ...
            color = y_true.argmax(axis=1)
            X = test_embeddings.cpu().numpy()
            np.save("embedding_{}_magnn.npy".format(args.dataset),X)
            np.save("color_{}_magnn.npy".format(args.dataset),color)
            print(X.shape)
            print(color.shape)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--save-postfix', type=str,default='DBLP', help='Postfix for the saved model and result. Default is DBLP.')
    ...
    args = ap.parse_args()
    seed(123)
    run_model_DBLP(args,args.feats_type, args.hidden_dim, args.num_heads, args.attn_vec_dim, args.rnn_type,
